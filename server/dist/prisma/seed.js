...
exports.INITIAL_STICKERS = exports.INITIAL_BANNERS = exports.INITIAL_DAILY_TASK_POOL = exports.INITIAL_NPC_ORDERS = exports.INITIAL_BADGES = exports.RECIPES = exports.INITIAL_ITEMS = void 0;
...
exports.INITIAL_DAILY_TASK_POOL = [
...
async function seedDatabase() {
    ...
    for (const item of exports.INITIAL_ITEMS) {
        await client_1.default.item.upsert({
            where: { id: item.id },
            update: item,
            create: item
        });
    }
    for (const badge of exports.INITIAL_BADGES) {
        await client_1.default.badge.upsert({
            where: { id: badge.id },
            update: badge,
            create: badge
        });
    }
    for (const order of exports.INITIAL_NPC_ORDERS) {
        await client_1.default.npcOrder.upsert({
            where: { id: order.id },
            update: order,
            create: order
        });
    }
    for (const banner of exports.INITIAL_BANNERS) {
        await client_1.default.bannerCosmetic.upsert({
            where: { id: banner.id },
            update: banner,
            create: banner
        });
    }
    for (const sticker of exports.INITIAL_STICKERS) {
        await client_1.default.sticker.upsert({
            where: { id: sticker.id },
            update: sticker,
            create: sticker
        });
